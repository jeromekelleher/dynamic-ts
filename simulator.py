...
This version basically reproduces (but slightly simplifies) the approach
taken in tskit, where we periodically simplify.
"""
import random
import collections
import heapq
...
class Simulator(object):
    ...
    def run(self, num_generations, simplify_interval=1):
        for _ in range(num_generations):
            ...
            if self.time % simplify_interval == 0:
                ...
                # It's too hard to make sure the IDs all end up the same,
                # so we just check the number of nodes and edges are the same.
                # This is a pretty strong check in practise.
    ...
    def simplify(self):
        ...
        # the main loop at all. This doesn't seem to be the case in
            ...
            for left, right, X in overlapping_segments(S):
                if len(X) == 1:
                    ...
                    if output_ind is None:
                        output_ind = Individual(input_ind.time, False)
...
############################
# "New" implementation. This is a slightly different layout which looks
# at how we'd use the individuals and edges to build trees.
# Commenting it out for now to reduce confusion as it contains some
# stuff we'll want to merge in at some point.
############################
...
def main():
    ...
    seed = 1
    sim = Simulator(150, 20, death_proba=0.5, seed=seed)
    sim.run(200, 10)
    # ts = sim.export()
    # print(ts.draw_text())
